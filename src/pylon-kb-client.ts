...
export class PylonKBClient {
  ...
  // POST /attachments — uploads a file and returns an embeddable URL.
  // OpenAPI: response is CreateAttachmentResponseBody with Attachment { id, name, description, url }.
  // Request shape (multipart vs JSON) is not rendered in public docs at the time of writing —
  // we send multipart/form-data with a `file` field, which is the standard pattern. If Pylon
  // returns 4xx with a useful error, adjust the field name or content-type here.
  async uploadAttachment(
    data: Uint8Array,
    filename: string,
    mimeType = 'image/png',
  ): Promise<{ id: string; url: string; name?: string; description?: string }> {
    const url = `${this.baseUrl}/attachments`;
    const form = new FormData();
    // Cast through unknown to satisfy both Node and Worker type defs (DOM lib's
    // BlobPart wants ArrayBufferView<ArrayBuffer>; Node's Uint8Array<ArrayBufferLike> is wider).
    ...
    if (!res.ok) {
      throw new Error(
        `Pylon POST /attachments failed with ${res.status} ${res.statusText}: ${text}\n` +
          `Hint: this endpoint's request shape is unverified in public docs. ` +
          `If the server expects a different field name, content-type, or wrapper, update ` +
          `uploadAttachment in src/pylon-kb-client.ts.`,
      );
    }
    let parsed: unknown;
    try {
      ...
      throw new Error(`Pylon /attachments returned non-JSON body: ${text}`);
    }
    ...
    if (!attachment || typeof attachment.url !== 'string' || typeof attachment.id !== 'string') {
      throw new Error(`Pylon /attachments returned unexpected shape: ${text}`);
    }
    return attachment as { id: string; url: string; name?: string; description?: string };
  }
}
